...
+ 변수란?
```python
>>> a = [1, 2, 3] #리스트 자료형(객체)가 자동으로 메모리에 생성, 변수 a는 [1, 2, 3]이라는 리스트가 저장된 메모리의 주소를 가리키게 됨. 
>>> id(a) #파이썬 내장함수인 id함수를 통해 변수 a가 가리키고 있는 객체의 주소 확인.
43206096 #[1, 2, 3]이라는 리스트의 주소는 43206096임을 알 수 있음.
```
...
+ 연습문제
  ...
  >>> b = [1, 2, 3]
  >>> a is b
  False  # a, b 각각에 해당하는 리스트는 개별적인 객체이므로, 결과값으로 False가 리턴.
  ```
  ```python
  >>> a = [1, 2, 3]
  >>> b = [1, 2, 3]
  >>> a == b
  True  # 값의 일치 여부를 판별하는 == 연산자는 결과값으로 True 리턴.
  ```  
  2. 변수와 객체2
  ```python
  >>> a = [1, 2, 3]
  >>> b = a
  >>> a is b
  True  # a가 가리키는 리스트를 b 역시 가리키고 있음. 즉, a와 b 둘 다 동일한 리스트를 가리키므로 True 리턴.
  ```
  3. 객체의 변경
  ```python
  >>> a = b = [1, 2, 3]
  >>> a[1] = 4
  >>> print(b)
  [1, 4, 3]  # a와 b는 동일한 리스트 객체를 가리키고 있기 때문에, 변수 a를 통해 리스트 요소값을 변경하면 변수 b의 출력값 역시 동일하게 변경됨. 
  ```
  4. 리스트 복사1
  ```python
  >>> a = [1, 2, 3]
  >>> b = a[:]
  >>> a is b
  False  # a[:]는 리스트 요소값이 동일한 새로운 객체를 생성하므로, a와 b가 가리키는 객체가 다름. 그 결과 False 리턴
  ```
  5. 리스트 복사2
  ```python
  >>> a = [1, 2, 3]
  ...
  [1, 4, 3]
  >>> print(b)
  [1, 2, 3]  # a와 b는 서로 다른 객체를 가리키므로 a가 가리키는 리스트의 요소값을 변경해도 b는 영향을 받지 않음.
  ```
  6. 리스트 더하기와 extend
  ```python
  >>> a = [1, 2, 3]
  ...
  [1, 2, 3, 4, 5]
  >>> id(a)
  46763184  # 이전에 리스트 a가 저장되어 있던 주소와 다른 값 리턴.
  ```
  ```python
  >>> a = [1, 2, 3]
  >>> a = 46763104
  ...
  [1, 2, 3, 4, 5]
  >>> id(a)
  46763104  # 연산자 + 와 달리 a가 가리키는 객체의 주소값이 변하지 않고 그대로 유지됨.
  ```
  7. 리스트 복사3
  ```python
  >>> a = [1, [2, 3], 4]
  ...
  >>> print(b)
  [1, [5, 3], 4]
  # a와 b는 서로 다른 객체이지만, a와 b가 포함하고 있는데 [2, 3] 리스트는 동일한 객체. 따라서 a에 포함된 리스트의 값을 변경하면 b도 함께 변경됨.
  # b = a[:]와 같은 copy를 얕은 copy(shallow copy)라고 함. 1 depth까지는 복사하지만 예제와 같은 2 depth 이상의 리스트는 복사하지 못하기 때문.
  ```
  ```python
  >>> from copy import deepcopy  # 객체가 포함하고 있는 객체까지 copy하고 싶다면 깊은 copy(deep copy)를 사용해야 함.
  >>> a = [1, [2, 3], 4]
  >>> b = deepcopy(a)
  >>> a[1][0] = 5
  >>> a
